...
If a page still uses classic processes (as the legacy `demo/f107.sql` does for older APEX versions), either convert it to a Form Region or keep the column `VARCHAR2`/`CLOB`. In `demo/f9107.sql`, Page 3 (Maintain Project) and Page 7 (Form Region JTL) are both implemented as modern Form Regions with Form Initialization and Form Automatic Row Processing, fully supporting round-tripping of 21c/23ai+ native JSON columns. The full APEXlang source for application 9107 is also provided in `demo/apexlang/`. Also note the legacy `apex_json.to_xmltype`/`xmltable` extraction (the 11g section below) does not compile against JSON-typed columns (`PLS-306`) — use `JSON_TABLE`.
...
* Refreshed demo app: [demo/f9107.sql](demo/f9107.sql) and APEXlang source in [demo/apexlang](demo/apexlang) — APEX 26.1 export with Universal Theme refreshed, compatibility mode 26.1, legacy JavaScript/jQuery Migrate includes removed, and modern Form Regions (Page 3 and Page 7) demonstrating Form Initialization / Automatic Row Processing against JSON-typed `PX_PROJECTS2` columns. Supporting objects create those columns as native `JSON` and install views with `JSON_TABLE`. The original [demo/f107.sql](demo/f107.sql) remains for older APEX versions.
